...
# conventional needed for surface cells
bulk = sga.get_conventional_standard_structure()

# Generate surf slabs and output for visualisation
SlabGen = SlabGenerator(bulk, gb_plane, slab_min_t, slab_min_t, lll_reduce=False, center_slab=False,
                        in_unit_planes=True, primitive=True, max_normal_search=None, reorient_lattice=True)
...
for i in range(0, Na):
    for j in range(0, Nb):
        ...
        GBSlab = gengb(slabs[Grain1_slabid], slabs[Grain2_slabid], GB_x_shift, GB_y_shift,
                       GB_z_shift1, GB_z_shift2, MergeOn, merge_tol, MirrorX, MirrorY, MirrorZ)
        ...
        if GBSlab.is_valid(tol=MinBond):
            ...
            line = subprocess.check_output(
                ["grep", "rgy  w", "OUTCAR"]).decode('ASCII').split()
            E = line[6]
            file1.write("{:.4f} {:.4f} {:.6f}\n".format(
                float(i/Na), float((j/Nb)), float(E)))
        else:
            file1.write("{:.4f} {:.4f} {:.6f}\n".format(
                float(i/Na), float((j/Nb)), float(np.inf)))
